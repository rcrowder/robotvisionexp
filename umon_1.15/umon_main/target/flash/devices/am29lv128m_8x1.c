...
#include "cpu.h"
#include "flash.h"
#include "am29lv128m_8x1.h"

#define ftype               volatile unsigned char
#define Read_0000()         (*(ftype *)(fdev->base))
#define Read_0001()         (*(ftype *)(fdev->base+0x02))
#define Read_0002()         (*(ftype *)(fdev->base+0x1C))
#define Read_0003()         (*(ftype *)(fdev->base+0x1E))
...
#define SECTOR_ERASE(add) { \
        (*(ftype *)(fdev->base+0x0aaa) = 0xaa); \
        (*(ftype *)(fdev->base+0x0555) = 0x55); \
        (*(ftype *)(fdev->base+0x0aaa) = 0x80); \
        (*(ftype *)(fdev->base+0x0aaa) = 0xaa); \
        (*(ftype *)(fdev->base+0x0555) = 0x55); \
        (*(ftype *)add = 0x30); }
...
#define AUTO_SELECT() { \
        (*(ftype *)(fdev->base+0x0aaa) = 0xaa); \
        (*(ftype *)(fdev->base+0x0555) = 0x55); \
        (*(ftype *)(fdev->base+0x0aaa) = 0x90); }

#define READ_RESET() { \
        (*(ftype *)(fdev->base+0x0555) = 0xf0); }

/* Am29lv128m_8x1_erase():
 * Based on the 'snum' value, erase the appropriate sector(s).
 * Return 0 if success, else -1.
 */
int
Am29lv128m_8x1_erase(struct flashinfo *fdev,int snum)
{
    ulong   add;
    int ret, sector;
 ...
 */
int
Am29lv128m_8x1_write(struct flashinfo *fdev,ftype *dest,ftype *src,long bytecnt)
{
    int i, ret;
 ...
 */
int Am29lv128m_8x1_ewrite(struct flashinfo *fdev,ftype *dest,ftype *src,int bytecnt)
{
    volatile ulong  add;
    void    (*reset)();
    ftype   *src1, *dest1;
 ...
 */
int
Am29lv128m_8x1_type(struct flashinfo *fdev)
{
    volatile int    i;
    ftype   val;
    unsigned int    dev;
    ...
    dev   = ( Read_0000() & 0xff) << 24;  /* manufacturer ID */
    dev  |= ( Read_0001() & 0xff) << 16;  /* device ID */
    dev  |= ( Read_0002() & 0xff) <<  8;  /* device ID */
    dev  |= ( Read_0003() & 0xff) <<  0;  /* device ID */
...
struct flashdesc FlashNamId[] = {
    { AMD_29LV128M,   "AMD-29LV128M" }      /* AMD */
};

struct sectorinfo sinfo128[AMD_29LV128M_NUM_SECTORS];
...
FlashBankInit(struct flashinfo *fbnk, int snum)
{
    ...
    switch(fbnk->id) {
        case AMD_29LV128M:
            fbnk->sectorcnt = AMD_29LV128M_NUM_SECTORS ;
            ssize = AMD_29LV128M_SECTOR_SIZE * fbnk->width;
            fbnk->end = fbnk->base + (AMD_29LV128M_TOTAL_SIZE * fbnk->width) - 1;      /* 8MB each */
            break;
        default:
    ...
    return( AMD_29LV128M_NUM_SECTORS );                                  /* should this just return fbnk->sectorcnt? (that would be a more general solution) */
}
...
#ifdef FLASH_COPY_TO_RAM
    if (flashopload((ulong *)Am29lv128m_8x1_type,
        (ulong *)EndAm29lv128m_8x1_type,
        FlashTypeFbuf,sizeof(FlashTypeFbuf)) < 0)
        return(-1);

    if (flashopload((ulong *)Am29lv128m_8x1_erase,
        (ulong *)EndAm29lv128m_8x1_erase,
        FlashEraseFbuf,sizeof(FlashEraseFbuf)) < 0)
        return(-1);

    if (flashopload((ulong *)Am29lv128m_8x1_ewrite,
        (ulong *)EndAm29lv128m_8x1_ewrite,
        FlashEwriteFbuf,sizeof(FlashEwriteFbuf)) < 0)
        return(-1);

    if (flashopload((ulong *)Am29lv128m_8x1_write,
        (ulong *)EndAm29lv128m_8x1_write,
...
#else
    fbnk->fltype   = (int(*)())Am29lv128m_8x1_type;
    fbnk->flerase  = (int(*)())Am29lv128m_8x1_erase;
    fbnk->flwrite  = (int(*)())Am29lv128m_8x1_write;
    fbnk->flewrite = (int(*)())Am29lv128m_8x1_ewrite;
#endif
   fbnk->fllock = FlashLockNotSupported;

    fbnk->sectors = sinfo128;
