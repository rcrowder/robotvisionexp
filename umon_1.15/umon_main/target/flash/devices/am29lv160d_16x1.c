...
/* Am29lv160d_16x1_erase():
 * Based on the 'snum' value, erase the appropriate sector(s).
 * Return 0 if success, else -1.
 */
int
Am29lv160d_16x1_erase(struct flashinfo *fdev,int snum)
{
	ulong	add;
	int		ret;
...
/* EndAm29lv160d_16x1_write():
 * Function place holder to determine the end of the above function.
 */
void
EndAm29lv160d_16x1_write(void)
{}

/* Am29lv160d_16x1_ewrite():
 ...
 */
int
Am29lv160d_16x1_ewrite(struct flashinfo *fdev,
	uchar *dest,uchar *src,long bytecnt)
{
	int	i;
	ulong	add;
...
/* EndAm29lv160d_16x1_ewrite():
 * Function place holder to determine the end of the above function.
 */
void
EndAm29lv160d_16x1_ewrite(void)
{}


/* Am29lv160d_16x1_type():
 * Run the AUTOSELECT algorithm to retrieve the manufacturer and
 * device id of the flash.
 */
int
Am29lv160d_16x1_type(struct flashinfo *fdev)
{
	ushort	man, dev;
	ulong	id;
...
/* EndAm29lv160d_16x1_type():
 * Function place holder to determine the end of the above function.
 */
void
EndAm29lv160d_16x1_type(void)
{}
...
int	SectorSizes160DT[] = {	/* Top boot */
	...
	0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000,
	0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000,
	0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x08000,
	0x02000, 0x02000, 0x04000,
};

int	SectorSizes160DB[] = {	/* Bottom boot */
	0x04000, 0x02000, 0x02000, 0x08000, 0x10000, 0x10000, 0x10000, 0x10000,
	0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000,
	0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000,
	0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000, 0x10000,
...
struct sectorinfo sinfo160[sizeof(SectorSizes160DB)/sizeof(int)];
...
FlashInit()
{
...
#ifdef FLASH_COPY_TO_RAM
	if (flashopload((ulong *)Am29lv160d_16x1_type,
		(ulong *)EndAm29lv160d_16x1_type,
		FlashTypeFbuf,sizeof(FlashTypeFbuf)) < 0)
		return(-1);
	
	if (flashopload((ulong *)Am29lv160d_16x1_erase,
		(ulong *)EndAm29lv160d_16x1_erase,
		FlashEraseFbuf,sizeof(FlashEraseFbuf)) < 0)
		return(-1);
	
	if (flashopload((ulong *)Am29lv160d_16x1_ewrite,
		(ulong *)EndAm29lv160d_16x1_ewrite,
		FlashEwriteFbuf,sizeof(FlashEwriteFbuf)) < 0)
		return(-1);
	
	if (flashopload((ulong *)Am29lv160d_16x1_write,
		(ulong *)EndAm29lv160d_16x1_write,
	...
	fbnk->sectorcnt = (sizeof(SectorSizes160DB)/sizeof(int));
...
#else
	fbnk->fltype = Am29lv160d_16x1_type;
	fbnk->flerase = Am29lv160d_16x1_erase;
	fbnk->flwrite = Am29lv160d_16x1_write;
	fbnk->flewrite = Am29lv160d_16x1_ewrite;
#endif
	...
	if (fbnk->id == AMD29DL160DT) {
		sizetbl = SectorSizes160DT;
	}
	else if (fbnk->id == AMD29DL160DB) {
		sizetbl = SectorSizes160DB;
	}
	else {
