...
*/
int
Flasherase29F800B_16(struct flashinfo *fdev,int snum)
{
	ftype	val;
	ulong	add;
	int	ret, sector;
...
*/
int
Flashwrite29F800B_16(struct flashinfo *fdev,uchar *dest,uchar *src,long bytecnt)
{
	int		i, ret;
	long	cnt;
	uchar 	*src1;
	...
	if (NotAligned(dest)) {
		...
		buf[0] = *(dest-1);
		buf[1] = *src;
		Flashwrite29F800B_16(fdev,dest-1,buf,2);
		dest++; src++; bytecnt--;
	}
...
int
Flashewrite29F800B_16(struct flashinfo *fdev,ftype *dest,ftype *src,int bytecnt)
{
	int	i;
	ulong	add;
	void	(*reset)();
...
int
Flashtype29F800B_16(struct flashinfo *fdev)
{
	ftype	val;
	ushort	man, dev;
	ulong	id;
