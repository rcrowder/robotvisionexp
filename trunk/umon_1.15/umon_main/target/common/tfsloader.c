...
#ifdef USE_ALTERNATE_TFSLD_MEMCPY
extern int tfsld_decompress(char *src,int srclen,char *dest,int verbose);
#else
int
tfsld_decompress(char *src, int srclen, char *dest, int verbose)
{
	int outsize;
	 ...
	 */
	if (verbose <= 1) {
		if (inUmonBssSpace(dest, (char *)src+srclen))
			return(-1);
	}
		...
		if (ISLOADABLE(shdr->s_flags)) {
			...
			if (TFS_ISCPRS(fp)) {
				...
				outsize = tfsld_decompress((char *)shdr->s_scnptr+(int)fhdr,
					shdr->s_size,(char *)shdr->s_paddr, verbose);
				if (outsize == -1) {
					err++;
					shdr++;
					continue;
	...
	for (i=0;!err && i<ehdr->e_shnum;i++,shdr++) {
		...
		if (shdr->sh_type == SHT_NOBITS) {
			...
			if (TFS_ISCPRS(fp)) {
				...
				outsize = tfsld_decompress((char *)(ehdr)+shdr->sh_offset,
					size,(char *)sh_addr,verbose);
				if (outsize == -1) {
					err++;
					continue;
				}
