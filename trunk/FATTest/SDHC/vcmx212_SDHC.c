...
#include "vcmx212_regs.h"
#include "vcmx212_GPIO.h"
...
// http://www.rtems.com/ml/rtems-users/2005/december/msg00051.html 
...
int SDHC_Init(int iVerbose)
{
	...
	printf("SDHC_Init: Card capacity %dMB (%d bytes)\n", (uiCardSize/1024/1024), uiCardSize);
...
{
//	printf("SDHC_Read(0x%08X, %d, %d, %d)\n", (UINT)pBuffer, iBlockNum, iBlockCnt, iVerbose);
	...
	for (iCurrBlockCnt = 0; iCurrBlockCnt < iBlockCnt; iCurrBlockCnt++)
	{
		...
		while (Count > 0)
		{
			...
			if(uiStatus&0x000B)
			{
				printf("SDHC_Read: Read Error (Status = 0x%04X)\n", uiStatus);
				bRet = 0;
				goto StopReadClock;
			}
...
{
//	printf("SDHC_Write(0x%08X, %d, %d, %d)\n", (UINT)pBuffer, iBlockNum, iBlockCnt, iVerbose);
	...
	for (iCurrBlockCnt = 0; iCurrBlockCnt < iBlockCnt; iCurrBlockCnt++)
	{
		...
		while (!bWriteDone && Count > 0)
		{
			...
			if (uiStatus&0x0003)
			{
				printf("SDHC_Write: Write Error (Status = 0x%04X)\n", uiStatus);
				bRet = 0;
				goto SingleWriteEnd;
			}
			else
